...
function setRole(role) {
  ...
  [...checklistForm.elements].forEach((field) => {
    if (field.tagName === 'BUTTON') return;
    field.disabled = !canEdit;
  });

  generateBtn.disabled = !canEdit;
  document.getElementById('addChecklistBtn').disabled = !canEdit;
  document.querySelectorAll('.addActivityBtn,.deleteChecklistBtn').forEach((btn) => {
...
function renderLogistics(destination, travelers) {
  flightCard.textContent = `Round-trip estimate for ${travelers} traveler${travelers > 1 ? 's' : ''} to ${destination}.`;
  hotelCard.textContent = `4-star stay near central ${destination}, includes breakfast.`;
}

function getChecklist() {
  return JSON.parse(localStorage.getItem('tripChecklist') || '[]');
}
...
function renderChecklist() {
  ...
  items.forEach((item, index) => {
    ...
    del.className = 'deleteChecklistBtn';
...
form.addEventListener('submit', (event) => {
  ...
  try {
    ...
    renderLogistics(destination, travelers);
